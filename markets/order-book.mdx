...
An order book is a real-time list of all active [limit orders](/markets/limit-orders) in a market. It displays:

- **Buy orders**: Traders willing to buy HIT shares (or sell MISS shares).
- **Sell orders**: Traders willing to sell HIT shares (or buy MISS shares).
...
**Switching Between HIT and MISS**

At the top of the order book, you can toggle between "Trade HIT" and "Trade MISS" tabs to view the order book for either side of the market. The structure remains the same - Asks on top, Bids on bottom - but the orders shown will be for the selected outcome.
...
**Use a [Market Order](/markets/market-orders) when:**

- The spread is tight.
- There's enough liquidity at the best price.
- You want to trade immediately.

**Use a [Limit Order](/markets/limit-orders) when:**
...
When a [market order](/markets/market-orders) matches with a limit order, both are removed from the book.
