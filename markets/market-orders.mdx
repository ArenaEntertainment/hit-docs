...
A market order is an instruction to buy or sell shares instantly at the best price currently available in the [order book](/markets/order-book).
...
4. **Confirm** - Click "Buy HIT" or "Buy MISS" to execute immediately.

Your order matches with the best available [limit orders](/markets/limit-orders) in the order book, filling at progressively worse prices until your full dollar amount is spent.
...
**Example:**
You place a market order to buy \$100 worth of HIT shares. The order book shows:
...
<Warning>
  **Check liquidity first**: In thin markets, large market orders can experience
  ...
  book](/markets/order-book) depth before placing large orders.
</Warning>
...
- You want to wait for a better price (use [limit orders](/markets/limit-orders) instead).
...
Learn more about [Limit Orders](/markets/limit-orders).
