...
The signature message joins four parts with newline characters:

```text
timestamp + "\n" + method + "\n" + pathWithQuery + "\n" + body
```

`pathWithQuery` includes `/api/v1` and the query string exactly as sent, and excludes the host. `body` is the exact string you send, or an empty string when there is no body. The older concatenated message is still accepted; see [Authentication](/developers/authentication) for both forms and for the requests that accept only the older one.
...
User-specific streams require API key authentication after connecting to `/user`, signed with its own message rather than the REST one. See [WebSockets](/developers/websockets).
