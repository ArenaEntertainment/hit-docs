...
These apply per socket on the public namespace. Both order book limits answer with an `orderbook-error` rather than dropping your message silently, so you never have to infer a refusal from books that do not arrive.
