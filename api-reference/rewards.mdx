...
Each `RewardsLeaderboardEntry` contains integer `rank`, `userId`, optional `displayName`, boolean `isCurrentUser`, and string `earnedUsdc`. Ties share a rank. No raw scoring units, reward share, projection, trading P/L or volume values are returned.
