...
`GlobalTrade` contains `id`, microsecond `matchedAt` and nullable `confirmedAt`, trader display fields, `side`, `sideRaw`, nullable `outcome`, `notionalUsd`, `price`, `size`, `tokenId`, market fields, event fields, and `isMultiMarket`.
...
| `user-mp-claim-update` | `RewardClaimUpdate` | Rewards claim dispatch notification. The legacy event name remains live; raw rewards accrual updates are no longer exposed. |
